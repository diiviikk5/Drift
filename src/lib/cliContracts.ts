...
export interface CliExportRequest {
	kind: "export";
	/** Absolute path to the .drift project file. */
	projectPath: string;
...
export interface CliRecordRequest {
	...
	/** When set, write a ready-to-export .drift project here after recording. */
...
export interface CliCaptionsRequest {
	kind: "captions";
	/** Absolute path to the .drift project file (updated in place). */
	projectPath: string;
...
export interface CliDoneResult {
	...
	durationMs?: number;
	/**
	 * Record: a ready-to-save .drift project object built by the runner.
