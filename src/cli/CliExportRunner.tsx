// Hidden-window runner for `drift export`. Loads an .drift project,
...
function replaceExtension(filePath: string, newExtension: string): string {
	return filePath.replace(/\.(drift|json)$/i, "") + newExtension;
}
...
async function runExport(request: CliExportRequest): Promise<CliDoneResult> {
	...
	if (!validateProjectData(loaded.project)) {
		throw new Error("Project file is not a valid .drift project");
	}
	...
	// Migrate the .drift project onto the AxcutDocument the native
