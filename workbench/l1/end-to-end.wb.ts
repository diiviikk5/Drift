...
// `chat-service.ts`, one storey above `invokeDriftAgent`, so a test that
...
import { EXPECTED_TOOL_COUNT, DRIFT_TOOLS, PHANTOM_TOOLS } from "../lib/prompts";
...
describe("the context the model actually receives", () => {
	...
	it("is our tool surface and nothing else", async () => {
		...
		for (const name of DRIFT_TOOLS) expect(run.wire.toolNames).toContain(name);
