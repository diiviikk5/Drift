...
Headless command-line interface for recording the screen and exporting `.drift`
...
```bash
# macOS
/Applications/Openscreen.app/Contents/MacOS/Openscreen export demo.drift -o demo.mp4
# Windows
"C:\Program Files\Openscreen\Openscreen.exe" export demo.drift -o demo.mp4
```
...
```bash
drift record --duration 30 --project demo.drift --json
drift record --window "My App" --mic --system-audio
drift record --display 1 --cursor system
```
...
| `--project <out.drift>` | Write a ready-to-export project file when done |
...
```bash
drift sources          # human-readable
drift sources --json
drift sources -o sources.json   # straight to a file
```
...
and OpenGL arrive ahead of the JSON, so `drift sources --json | jq` fails.
...
```bash
drift sources --json | jq '.sources.displays'   # stdout: inside the envelope
drift sources -o s.json && jq '.displays' s.json # file: the payload itself
```
...
```bash
drift export demo.drift                    # format/quality from the project
drift export demo.drift -o out.mp4 --quality source
drift export demo.drift -o out.gif --gif-fps 20 --gif-size large
drift export demo.drift --json | while read line; do ...; done
```
...
Keep `.drift` files beside their media (or record via the CLI, which uses
...
```bash
drift pack demo.drift --out bundle/
```
...
```bash
drift captions demo.drift --min-words 2 --max-words 7
drift export demo.drift -o demo.mp4   # subtitles are burned in
```
...
drift record --window "MyProduct" --duration 20 --project demo.drift --json

# 2. Edit the project: add a zoom and a caption (plain JSON)
node -e '
  const fs = require("fs");
  const p = JSON.parse(fs.readFileSync("demo.drift", "utf8"));
  ...
  fs.writeFileSync("demo.drift", JSON.stringify(p, null, 2));
'
...
drift export demo.drift -o demo.mp4 --auto-zoom --audio voice.m4a --audio-mode replace --json
```
