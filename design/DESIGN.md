...
> Source: tokens extracted from `drift-editor-2.html` (canonical)
...
| File | Role |
|---|---|
| `drift-editor-2.html` | Latest editor (light + dark themes in one file). Canonical source of tokens. |
| `drift-editor.html` | First editor pass. Light-only. |
| `editor.html` | Earlier experimental editor with red REC accent. |
| `drift-landing.html` | Marketing landing page. |
...
- Landing page (`drift-landing.html`) is light-only; consider a dark variant once product photography is sourced.
