...
const OUTPUT_DIR =
	...
	path.join(os.tmpdir(), `drift-cursor-native-${Date.now()}`);
...
function spawnPowerShell(script, { onStdout, onStderr } = {}) {
	const scriptPath = path.join(
		os.tmpdir(),
		`drift-powershell-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`,
	);
...
public static class DriftCursorDiagnosticInterop {
...
$standardCursors = @{
    arrow = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32512))
    text = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32513))
    wait = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32514))
    crosshair = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32515))
    'up-arrow' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32516))
    'resize-nwse' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32642))
    'resize-nesw' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32643))
    'resize-ew' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32644))
    'resize-ns' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32645))
    move = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32646))
    'not-allowed' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32648))
    pointer = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32649))
    'app-starting' = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32650))
    help = [DriftCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32651))
}
...
function Get-CursorAsset($cursorHandle, $cursorId) {
    $copiedHandle = [DriftCursorDiagnosticInterop]::CopyIcon($cursorHandle)
    if ($copiedHandle -eq [IntPtr]::Zero) {
        return $null
    }

    $iconInfo = New-Object DriftCursorDiagnosticInterop+ICONINFO
    $hasIconInfo = [DriftCursorDiagnosticInterop]::GetIconInfo($copiedHandle, [ref]$iconInfo)
    ...
    try {
        ...
        if ($hasIconInfo) {
            if ($iconInfo.hbmMask -ne [IntPtr]::Zero) {
                [DriftCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmMask) | Out-Null
            }
            if ($iconInfo.hbmColor -ne [IntPtr]::Zero) {
                [DriftCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmColor) | Out-Null
            }
        }
        [DriftCursorDiagnosticInterop]::DestroyIcon($copiedHandle) | Out-Null
    }
}

[DriftCursorDiagnosticInterop]::InstallMouseHook() | Out-Null
[DriftCursorDiagnosticInterop]::GetAsyncKeyState(0x01) | Out-Null
...
while ($true) {
    ...
    $mouseEvents = [DriftCursorDiagnosticInterop]::ConsumeMouseButtonEvents()
    $cursorInfo = New-Object DriftCursorDiagnosticInterop+CURSORINFO
    $cursorInfo.cbSize = [Runtime.InteropServices.Marshal]::SizeOf([type][DriftCursorDiagnosticInterop+CURSORINFO])

    if (-not [DriftCursorDiagnosticInterop]::GetCursorInfo([ref]$cursorInfo)) {
    ...
    $leftButtonState = [DriftCursorDiagnosticInterop]::GetAsyncKeyState(0x01)
...
public static class DriftMouseDiagnosticInterop {
    [DllImport("user32.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
...
for ($i = 0; $i -lt $points.Count; $i++) {
    $point = $points[$i]
    [DriftMouseDiagnosticInterop]::SetCursorPos($point.x, $point.y) | Out-Null
    if ($i -eq [int]([Math]::Floor($points.Count / 2))) {
        [DriftMouseDiagnosticInterop]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 12
        [DriftMouseDiagnosticInterop]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    }
...
<title>Drift native cursor diagnostic</title>
<style>
...
<body>
<main>
<h1>Drift native cursor diagnostic</h1>
<div class="metrics">
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Drift native cursor real capture diagnostic</title>
<style>
