...
const MAC_REQUIRED = [
	...
	{
		match: (name) => name === "drift-screencapturekit-helper",
...
const LINUX_REQUIRED = [
	...
	{
		match: (name) => name === "drift-pipewire-helper",
	...
	if (libs.length === 0) {
		throw new Error(
			"Refusing to package an incomplete Linux payload.\n\n" +
				...
				"      without it: drift-pipewire-helper dies in ld.so, so capture never starts\n" +
 ...
 */
const SYMBOL_FLOOR_MODE = process.env.DRIFT_SYMBOL_FLOOR ?? "";
...
function hostSymbolCeiling() {
	...
	if (missing.length > 0) {
		throw new Error(
			`DRIFT_SYMBOL_FLOOR=host could not read ${missing.join(", ")} from this machine.\n\n` +
 ...
 * DRIFT_SYMBOL_FLOOR here rather than at module load, so a stray value cannot
...
function resolveSymbolCeiling() {
	...
	if (SYMBOL_FLOOR_MODE !== "host") {
		throw new Error(
			`DRIFT_SYMBOL_FLOOR=${SYMBOL_FLOOR_MODE} is not a value this guard knows.\n\n` +
	...
	if (process.env.CI) {
		throw new Error(
			"DRIFT_SYMBOL_FLOOR=host is refused under CI.\n\n" +
		...
		console.log(
			`[before-pack] symbol-version ceiling taken from THIS MACHINE, not the pinned floor:\n` +
				`  ${Object.entries(ceiling)
					...
					.map(([prefix, max]) => `${prefix}_${max}`)
					.join(", ")})\n` +
				"  DRIFT_SYMBOL_FLOOR=host is set. The package this produces may not start on\n" +
		...
		"Refusing to package binaries that need a newer glibc or libstdc++ than the oldest\n" +
			...
			(pinned
				? "Building a package to run on THIS machine rather than to release? Set\n" +
					"DRIFT_SYMBOL_FLOOR=host, which compares against your own glibc instead of\n" +
...
exports.default = async function beforePack(context) {
	...
	if (platform === "linux") {
		...
		checkCompositorAddonFreshness(
			path.join(dir, "drift-pipewire-helper"),
			FIX_LINUX_HELPER,
