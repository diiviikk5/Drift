...
//
// Env overrides:
//   DRIFT_WHISPER_SERVER_EXE  helper binary (default: the staged one for this host)
//   DRIFT_WHISPER_MODEL       GGML model    (default: the userData cache location)
...
const BIN =
	process.env.DRIFT_WHISPER_SERVER_EXE ??
	path.join(
		ROOT,
		"electron",
		"native",
...
const MODEL = process.env.DRIFT_WHISPER_MODEL ?? defaultModelPath();
 ...
 * `name` ("drift") in dev and the productName once packaged — *not*
...
function defaultModelPath() {
	...
	const candidates = roots.flatMap((root) =>
		["drift", "Openscreen", "Electron"].map((appName) => path.join(root, appName, file)),
	);
	...
	for (const [label, p] of [
		...
		if (!fs.existsSync(p)) {
			...
			if (label === "helper binary") {
				...
				console.error(
					"       Run a transcription in the app once, or set DRIFT_WHISPER_MODEL.",
				);
			}
			process.exit(1);
		}
