...
const DURATION_MS = Number(process.env.DRIFT_WGC_TEST_DURATION_MS ?? 5000);
const WITH_SYSTEM_AUDIO =
	process.env.DRIFT_WGC_TEST_SYSTEM_AUDIO === "true" ||
	process.argv.includes("--system-audio");
const WITH_MICROPHONE =
	process.env.DRIFT_WGC_TEST_MICROPHONE === "true" ||
	process.argv.includes("--microphone") ||
	process.argv.includes("--mic");
const WITH_WINDOW =
	process.env.DRIFT_WGC_TEST_WINDOW === "true" || process.argv.includes("--window");
const WITH_WEBCAM =
	process.env.DRIFT_WGC_TEST_WEBCAM === "true" || process.argv.includes("--webcam");
const CAPTURE_CURSOR =
	process.env.DRIFT_WGC_TEST_CAPTURE_CURSOR === "true" ||
	process.argv.includes("--capture-cursor");
const WITH_SOFTWARE_ENCODER =
	process.env.DRIFT_WGC_TEST_SOFTWARE_ENCODER === "true" ||
	process.argv.includes("--software-encoder");
const WITH_SOFTWARE_FALLBACK =
	process.env.DRIFT_WGC_TEST_SOFTWARE_FALLBACK === "true" ||
	process.argv.includes("--software-fallback");
const INJECT_DEFAULT_SINK_WRITER_FAILURE_ENV =
	"DRIFT_WGC_TEST_INJECT_DEFAULT_SINK_WRITER_FAILURE_ONCE";
const INJECTION_MARKER = "TEST-ONLY: Injected default sink-writer creation failure";
const STALL_READBACK_ENV = "DRIFT_WGC_TEST_STALL_READBACK_MS";
/**
...
const WITH_STALLED_READBACK =
	process.env.DRIFT_WGC_TEST_STALL_READBACK === "true" ||
	process.argv.includes("--stall-readback");
const STALL_READBACK_MS = Number(process.env[STALL_READBACK_ENV] ?? 60_000);
const STALL_FRAME_CALLBACK_ENV = "DRIFT_WGC_TEST_STALL_FRAME_CALLBACK_MS";
/**
 * Reproduces getdrift/drift#460 on ordinary hardware: stalls the WGC
...
const WITH_STALLED_FRAME_CALLBACK =
	process.env.DRIFT_WGC_TEST_STALL_FRAME_CALLBACK === "true" ||
...
const STOP_BUDGET_ENV = "DRIFT_WGC_STOP_BUDGET_MS";
/**
...
const outputPath = path.join(
	os.tmpdir(),
	`drift-wgc-helper-${WITH_WEBCAM ? "webcam" : WITH_WINDOW ? "window" : WITH_SYSTEM_AUDIO || WITH_MICROPHONE ? "audio" : "video"}-${process.pid}-${Date.now()}-${randomUUID()}.mp4`,
);
...
const config = {
	...
	microphoneDeviceId: process.env.DRIFT_WGC_TEST_MICROPHONE_DEVICE_ID ?? "default",
	microphoneDeviceName: process.env.DRIFT_WGC_TEST_MICROPHONE_DEVICE_NAME ?? "",
	microphoneGain: 1.4,
	webcamEnabled: WITH_WEBCAM,
	webcamDeviceId: process.env.DRIFT_WGC_TEST_WEBCAM_DEVICE_ID ?? "",
	webcamDeviceName: process.env.DRIFT_WGC_TEST_WEBCAM_DEVICE_NAME ?? "",
	webcamDirectShowClsid: resolveDirectShowWebcamClsid(
		process.env.DRIFT_WGC_TEST_WEBCAM_DEVICE_NAME ?? "",
	),
	webcamWidth: 640,
	webcamHeight: 360,
	webcamFps: 30,
...
// The regression check for getdrift/drift#460: a frame callback
